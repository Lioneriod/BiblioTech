    ...
    class ActivityBuscaLivros : AppCompatActivity() {
        ...
        private fun buscarLivroAleatorio() {
            ...
            collectionRef.get()
                .addOnSuccessListener { document ->
                    val listaDeIds = document.documents.map { it.id }

                    if (listaDeIds.isNotEmpty()) {
                        val livroIdSorteado = listaDeIds.random()

                        abrirDetalhesLivro(livroIdSorteado, usuarioId)
                    } else {
                        Toast.makeText(
                            this,
                            "Nenhum livro encontrado para sortear",
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
        ...
        private fun abrirDetalhesLivro(livroId: String, usuarioId: String) {
            val intent = Intent(this, ActivityDetalhesLivro::class.java)
